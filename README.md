...
- 共 1086 道内置唯一题目，题库启动时自动校验并同步到 SQLite
...
1086 道是随项目分发的内置题目数量，不是平台的最终题量。用户生成并共享的 AI 题会保存在服务端，题目总量可以随着用户使用持续增加。当前 AI 题通过“其他用户生成的 AI 题”入口使用，不会自动冒充官方题目，也不会直接混入内置题库或正式模拟考试题池。
