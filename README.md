...
- 共 1070 道内置唯一题目，题库启动时自动校验并同步到 SQLite
...
1070 道是随项目分发的内置题目数量，不是平台的最终题量。用户生成并共享的 AI 题会保存在服务端，题目总量可以随着用户使用持续增加。当前 AI 题通过“其他用户生成的 AI 题”入口使用，不会自动冒充官方题目，也不会直接混入内置题库或正式模拟考试题池。
...
- 共享 AI 题库页面、题目质量反馈、共享开关和贡献统计
- 管理员面板：独立管理员 AI 配置、按知识点扩充共享题库、高重合题扫描与删除、用户封禁和操作审计
...
练习共享题并查看答案后，可以反馈“有帮助”“答案有问题”“表述不清”或“题目重复”。反馈按用户去重保存，用于展示题目质量和后续整理；反馈不会公开账号、API Key 或个人学习记录。
