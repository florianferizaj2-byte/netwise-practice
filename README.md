...
一个面向网络认证学习的刷题平台，支持按证书练习、错题复习、知识点掌握度追踪和限时模拟考试。

当前内置：

- 软考中级网络工程师：506 道题
- HCIA-Datacom V2.0：400 道题
- 共 906 道唯一题目，题库启动时自动校验并同步到 SQLite
...
API Key 只在服务端解密和调用时短暂存在，不会回传前端，也不会写入日志。AI 生成题会先经过 Schema 校验和独立质量审核，合格后才进入当前账号的训练队列，并标记为“AI 生成练习题”。
...
这是一个小规模学习应用，不是官方考试系统。题库中的原创题、仿真题、用户提供资料和 AI 生成题应以页面标注为准；AI 内容也不能保证完全无误。正式考试信息、报名时间和考试规则请以对应认证机构的最新公告为准。
